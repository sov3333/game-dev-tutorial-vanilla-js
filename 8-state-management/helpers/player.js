...
export default class Player {
    ...
    update(input){
        ...
        // vertical movement
        this.y += this.vy;
        if (!this.onGround()){ // if player is not standing on ground
            this.vy += this.weight;
        } else {
            this.vy = 0;
        }
        if (this.y > this.gameHeight - this.height) this.y = this.gameHeight - this.height; // optional check to not let char fall through floor
